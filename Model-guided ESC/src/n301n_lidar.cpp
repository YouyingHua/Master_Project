...
void n301n_lidar::poll(sensor_msgs::LaserScan::Ptr scan, int version_num)
{
    uint8_t temp_char;
                        ...
                        if(raw_bytes[i] == 0xFA && raw_bytes[i+1] == (0xA0+i/22)){
                            ...
                            for(uint16_t j = i+4; j < i+20; j=j+4) {
                                ...
                                uint16_t intensity_width = byte2;
  								uint16_t intensity = byte3;

                                if (index < 20 || index > 67) {   //change here
                                    scan->ranges[index] = 0;
                    ...
                    for(uint16_t i = 0; i < raw_bytes.size(); i=i+22) {
                        ...
                        if(raw_bytes[i] == 0xFA && raw_bytes[i+1] == (0xA0+i/22)){
                            ...
                            for(uint16_t j = i+4; j < i+20; j=j+4) {
                                ...
                                scan->intensities[359 - index] = 100 * intensity_width;
                            }
