...
class MobileTwist:
    ...
    count_J1 = 0
    count_J2 = 0
    SavefilespeedX = open("/home/nvidia/Desktop/catkin_ws/src/robot_formation/LS02A/data/OD13/speedX.txt", "a+")
    SavefilespeedY = open("/home/nvidia/Desktop/catkin_ws/src/robot_formation/LS02A/data/OD13/speedY.txt", "a+")
    SavefileRd = open("/home/nvidia/Desktop/catkin_ws/src/robot_formation/LS02A/data/OD13/Rd.txt", "a+")
    SavefileLR = open("/home/nvidia/Desktop/catkin_ws/src/robot_formation/LS02A/data/OD13/LR.txt", "a+")
    SavefileT = open("/home/nvidia/Desktop/catkin_ws/src/robot_formation/LS02A/data/OD13/time.txt", "a+")
    SavefilePred = open("/home/nvidia/Desktop/catkin_ws/src/robot_formation/LS02A/data/OD13/pred.txt", "a+")

    def __init__(self):   
        self.pub_twist = rospy.Publisher('cmd_vel',Twist,queue_size=1)
        self.subscriber_data = rospy.Subscriber("costpara", Costpara, self.callback1,queue_size=1)
        self.subscriber_pred = rospy.Subscriber("pred",std_msgs.msg.Int16, self.callback2, queue_size=1)
	self.subscriber_Rd = rospy.Subscriber("Rd", std_msgs.msg.Float32, self.callback_Rd, queue_size=1)
	...
	self.SavefilePred.write(str(self.pred)+"\n")
        ...
        if (self.flag_initial == 0):# or (abs(error_r)>= 2):
            self.publisher_ctl.publish("ESC_Re") #"Lookup"
            self.flag_initial = 1#0
	...
	self.sum = 0.15*self.err + 0.15*(self.err_last +self.err) + 0.2*(self.err - self.err_last)#-0.25,0.15
		...
		self.flag_initial = 2
                ...
                self.twist.linear.x = self.speed * self.sum *0.5; self.twist.linear.y = 0
        else:
	    if self.Rd <= -2:
		...
		elif abs(self.sum) < 0.3:
		    self.twist.linear.x = self.sum * self.speed
		elif abs(self.sum) < 0.6:
